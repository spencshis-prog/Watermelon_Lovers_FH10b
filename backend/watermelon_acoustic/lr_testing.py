...
def test_all_feature_models(models_dir, feature_extraction_base_dir, report_path):
    ...
    loads the corresponding final model (named model_<NR>_<FE>_<reg>.keras),
    ...
    Writes a report and produces grid plots:
      1. Four heatmaps (MAE, MSE, RMSE, R²)
    ...
    # For each regularization option, you can either loop here or call the function separately.
    # For this example, we assume one regularization option per run (the model name includes it).
    # We parse the regularization part from the model name (by splitting on '_' and taking the last element).
    for nr in nr_list:
        ...
        for fe in fe_list:
            ...
            # Here, we assume that during training the regularization was appended to the model name.
            # You might run separate tests for each regularization option.
            for reg in ["none", "lasso", "ridge", "ElasticNet"]:
                model_name = f"lr_{nr}_{fe}_{reg}.keras"
                model_path = os.path.join(models_dir, model_name)
                if not os.path.exists(model_path):
                    print(f"[TE] No model found for {nr}, {fe}, reg={reg} at {model_path}. Skipping.")
                    continue
    ...
    with open(report_path, "w") as f:
        for (nr, fe, reg), m in results.items():
...
def plot_results_grid(results):
    ...
    nr_list = sorted(list(set([k[0] for k in keys])))
    fe_list = sorted(list(set([k[1] for k in keys])))
    reg_list = sorted(list(set([k[2] for k in keys])))

    # For simplicity, we can produce separate grids for each regularization option.
    for reg in reg_list:
        print(f"[TE] Plotting results for Regularization: {reg}")
        # Filter results for current reg
        filtered = {(nr, fe): results[(nr, fe, reg)] for (nr, fe, r) in keys if r == reg}
        n_nr = len(nr_list)
        n_fe = len(fe_list)
        ...
        fig1.suptitle(f"Hold-out Test Metrics Heatmaps (Regularization: {reg})", fontsize=12)
        ...
        # Optionally, you can save figures here.
