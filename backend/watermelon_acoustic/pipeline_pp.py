...
def proceed(USE_QILIN=True, USE_LAB=False, USE_SEPARATE_TEST=False, TEST_SPLIT_RATIO=0.15):
    ...
    if USE_QILIN and USE_LAB:
        func.combine_folders(qilin_standard_dir, lab_standard_dir, combined_standard_dir)
    elif USE_QILIN:
    ...
    func.clear_output_directory(noise_reduction_dir)
    ...
    func.clear_output_directory(feature_extraction_base_dir)
    ...
    for technique in os.listdir(noise_reduction_dir):
        ...
        if os.path.isdir(technique_path):
            ...
            func.clear_output_directory(output_feat_dir)
