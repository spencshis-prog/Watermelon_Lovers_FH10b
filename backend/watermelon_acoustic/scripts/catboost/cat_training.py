...
def build_cat_model(hyper_tuning="default"):
    """
    Returns a CatBoostRegressor or a CV wrapper depending on the hyper_tuning strategy.
    """
    if hyper_tuning == "default":
        # Basic default CatBoost model.
        return CatBoostRegressor(random_state=42, silent=True)
        ...
        print(f"[CAT] Warning: Unknown hyper_tuning '{hyper_tuning}' requested. Using default CatBoostRegressor.")
        return CatBoostRegressor(random_state=42, silent=True)


def kfold_train_and_evaluate_cat(X, y, n_splits=5, epochs=20, batch_size=16, hyper_tuning="default"):
    """
    Performs K-fold cross-validation on (X, y) using a CatBoost model.
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        model = build_cat_model(hyper_tuning)
        ...
        if hasattr(model, 'best_estimator_'):
            final_est = model.best_estimator_
        ...
        print(f"[CAT] [Fold {fold_idx + 1}] MAE={mae:.2f}, RMSE={rmse:.2f}, R2={r2:.2f}")
...
def kfold_train_feature_set_cat(feature_folder, models_output_dir,
    ...
    then trains a final CatBoost model on the remaining data.
    ...
    functions.green_print(f"\n[CAT] K-fold training for [NR: '{nr_method}', FE: '{feat_method}', HT: '{hyper_tuning}']")

    X, y, fnames = functions.load_feature_data(feature_folder)
    if len(X) == 0:
        print(f"[CAT] No data in {feature_folder}. Skipping.")
        return None
    ...
    if os.path.exists(test_folder) and os.listdir(test_folder):
        ...
        print(
            f"[CAT] Using pre-split hold-out: {len(fnames_test)} test samples, {len(fnames_train_val)} training samples")
    else:
        ...
        print(f"[CAT] Hold-out test set created with {len(fn_test)} samples")

    # K-fold evaluation on the training+validation data.
    robust_metrics = kfold_train_and_evaluate_cat(X_train_val, y_train_val, n_splits=n_splits,
                                                  epochs=epochs, batch_size=batch_size,
                                                  hyper_tuning=hyper_tuning)
    mae_avg, mse_avg, rmse_avg, r2_avg = robust_metrics
    print(
        f"[CAT] [{nr_method} - {feat_method}] K-fold average -> MAE={mae_avg:.2f}, RMSE={rmse_avg:.2f}, R2={r2_avg:.2f}")

    # Train final CatBoost model on all training+validation data.
    final_model = build_cat_model(hyper_tuning)
    final_model.fit(X_train_val, y_train_val,
                    early_stopping_rounds=10,
                    verbose=False)
    if hasattr(final_model, 'best_estimator_'):
        final_model = final_model.best_estimator_
    print("[CAT] Final model parameters:", functions.relevant_params(final_model, "cat", hyper_tuning))
    model_name = f"cat_{nr_method}_{feat_method}_{hyper_tuning}.pkl"
    model_path = os.path.join(models_output_dir, model_name)
    os.makedirs(models_output_dir, exist_ok=True)
    joblib.dump(final_model, model_path)
    print(f"[CAT] Final model saved to {os.path.relpath(model_path, os.getcwd())}")
    return mae_avg, mse_avg, rmse_avg, r2_avg


def kfold_train_all_feature_models_cat(feature_extraction_base_dir, models_output_dir, report_kfold_path,
    ...
    for nr in nr_folders:
        ...
        for feat in feat_folders:
            ...
            metrics = kfold_train_feature_set_cat(feat_folder, models_output_dir,
    ...
    for (nr, feat, ht), (mae_avg, mse_avg, rmse_avg, r2_avg) in results.items():
        print(f"[CAT] {nr}-{feat} ({ht}) => MAE={mae_avg:.2f}, RMSE={rmse_avg:.2f}, R2={r2_avg:.2f}")
    print("[CAT] K-fold training completed for all feature sets.")
    ...
    print(f"[CAT] Report saved to {os.path.relpath(report_kfold_path, os.getcwd())}")
