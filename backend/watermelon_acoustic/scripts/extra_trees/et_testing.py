...
def test_model_on_holdout_et(model_path, test_folder):
    """
    Loads a final ExtraTrees model and evaluates it on the hold-out test set.
    Expects test_folder to contain the held-out .npy feature files.
    Returns a dictionary of test metrics and predictions.
    """
    if not os.path.exists(test_folder):
        print(f"[ET-TE] Test folder {test_folder} does not exist. Skipping.")
        return None

    X, y, _ = load_feature_data(test_folder)
    if len(X) == 0:
        print("[ET-TE] No test data found.")
        return None
...
def test_all_et_models(models_dir, feature_extraction_base_dir, report_path):
    """
    Iterates over each (NR, FE, hyper-tuning) combination in feature_extraction_base_dir,
    loads the corresponding final model (named et_<NR>_<FE>_<ht>.pkl),
    ...
    for nr in nr_list:
        ...
        for fe in fe_list:
            ...
            for ht in ["default", "grid", "random", "optuna"]:
                model_name = f"et_{nr}_{fe}_{ht}.pkl"
                model_path = os.path.join(models_dir, model_name)
                rel_model_path = os.path.relpath(model_path, project_root)
                if not os.path.exists(model_path):
                    print(f"[ET-TE] No model found for {nr}, {fe}, ht={ht} at {rel_model_path}. Skipping.")
                    continue
                print(
                    f"[ET-TE] Evaluating model {model_name} on test set from {os.path.relpath(test_folder, project_root)}")
                metrics = test_model_on_holdout_et(model_path, test_folder)
    ...
    print(f"[ET-TE] Report saved to {rel_report_path}")

    plot_results_grid_et(results, project_root)
...
def parse_report(report_path):
    ...
    if not os.path.exists(report_path):
        print(f"[ET-Parse] Report file {report_path} not found.")
        return results
    ...
    while idx < len(lines):
        ...
        try:
            ...
            print(f"[ET-Parse] Error parsing header: {header}")
            idx += 1
            continue
        ...
        try:
            ...
            print(f"[ET-Parse] Error parsing metrics for {header}")
        idx += 5
    return results


def plot_results_grid_et(results, project_root):
    """
    ...
    """
    base_save_dir = os.path.join(project_root, "../../output", "testing_et")
    ...
    report_kfold_path = os.path.join(project_root, "../../output", "testing_et", "report_kfold.txt")
    kfold_results = parse_report(report_kfold_path)
    if kfold_results:
        print(f"[ET-TE] Parsed K-Fold report from {os.path.relpath(report_kfold_path, project_root)}")
    else:
        print("[ET-TE] No valid K-Fold report found; only hold-out heatmaps will be plotted.")

    et_ht_options = sorted(list(set([ht for (_, _, ht) in results.keys()])))
    for ht in et_ht_options:
        print(f"[ET-TE] Plotting results for Hyper-tuning: {ht}")
        ...
        if kfold_filtered:
            ...
            heatmap_path = os.path.join(heatmaps_dir, f"et_{ht}_heatmap.png")
            rel_heatmap_path = os.path.relpath(heatmap_path, project_root)
            fig1.savefig(heatmap_path, dpi=300)
            print(f"[ET-TE] Saved combined heatmap to {rel_heatmap_path}")
            plt.close(fig1)
        else:
            ...
            rel_heatmap_path = os.path.relpath(heatmap_path, project_root)
            fig1.savefig(heatmap_path, dpi=300)
            print(f"[ET-TE] Saved hold-out heatmap to {rel_heatmap_path}")
            plt.close(fig1)
        ...
        for i, nr in enumerate(scatter_nr):
            for j, fe in enumerate(scatter_fe):
                ...
                if key in holdout_filtered:
                    m = holdout_filtered[key]
                    scatter_data[(i, j)] = (m["y_true"], m["y_pred"]) if "y_true" in m and "y_pred" in m else (
                    np.array([]), np.array([]))
                    residual_data[(i, j)] = (
                    m["y_pred"], m["y_true"] - m["y_pred"]) if "y_true" in m and "y_pred" in m else (
                    np.array([]), np.array([]))
        ...
        for i in range(n_nr_scatter):
            for j in range(n_fe_scatter):
                ax = axes2[i][j]
                if (i, j) in scatter_data and scatter_data[(i, j)][0].size > 0:
        ...
        pred_vs_actual_path = os.path.join(pred_vs_actual_dir, f"et_{ht}_predicted_vs_actual.png")
        rel_pred_vs_actual_path = os.path.relpath(pred_vs_actual_path, project_root)
        fig2.savefig(pred_vs_actual_path, dpi=300)
        print(f"[ET-TE] Saved predicted vs. actual plot to {rel_pred_vs_actual_path}")
        plt.close(fig2)
        ...
        for i in range(n_nr_scatter):
            for j in range(n_fe_scatter):
                ax = axes3[i][j]
                if (i, j) in residual_data and residual_data[(i, j)][0].size > 0:
        ...
        residual_plot_path = os.path.join(residual_plots_dir, f"et_{ht}_residual_plot.png")
        rel_residual_plot_path = os.path.relpath(residual_plot_path, project_root)
        fig3.savefig(residual_plot_path, dpi=300)
        print(f"[ET-TE] Saved residual plot to {rel_residual_plot_path}")
        plt.close(fig3)
