...
def test_model_on_holdout_xgb(model_path, test_folder):
    """
    Loads a final XGB model and evaluates it on the hold-out test set.
    Expects test_folder to contain the held-out .npy feature files.
    Returns a dictionary of test metrics and predictions.
    """
    if not os.path.exists(test_folder):
        print(f"[XGB-TE] Test folder {test_folder} does not exist. Skipping.")
        return None

    X, y, _ = load_feature_data(test_folder)
    if len(X) == 0:
        print("[XGB-TE] No test data found.")
        return None
...
def test_all_xgb_models(models_dir, feature_extraction_base_dir, report_path):
    """
    Iterates over each (NR, FE, hyper-tuning) combination in feature_extraction_base_dir,
    loads the corresponding final model (named xgb_<NR>_<FE>_<ht>.pkl),
    ...
    for nr in nr_list:
        ...
        for fe in fe_list:
            ...
            for ht in ["default", "grid", "random", "bayesian", "optuna"]:
                model_name = f"xgb_{nr}_{fe}_{ht}.pkl"
                model_path = os.path.join(models_dir, model_name)
                rel_model_path = os.path.relpath(model_path, project_root)
                if not os.path.exists(model_path):
                    print(f"[XGB-TE] No model found for {nr}, {fe}, ht={ht} at {rel_model_path}. Skipping.")
                    continue
                print(
                    f"[XGB-TE] Evaluating model {model_name} on test set from {os.path.relpath(test_folder, project_root)}")
                metrics = test_model_on_holdout_xgb(model_path, test_folder)
    ...
    print(f"[XGB-TE] Report saved to {rel_report_path}")

    plot_results_grid_xgb(results, project_root)
...
def parse_report(report_path):
    ...
    if not os.path.exists(report_path):
        print(f"[XGB-Parse] Report file {report_path} not found.")
        return results
    ...
    while idx < len(lines):
        header = lines[idx]
        try:
            # e.g. "Noise Reduction: bandpass, Feature Extraction: mfcc, Hyper-tuning: grid"
            parts = header.split(',')
...
def plot_results_grid_xgb(results, project_root):
    """
    Similar to your random forest code, produce:
      1) Heatmaps for hold-out test metrics
      2) A grid of Predicted vs. Actual scatter
      3) A grid of Residual plots
    We'll also see if we can parse a K-Fold report. For now, let's just do hold-out.
    """
    base_save_dir = os.path.join(project_root, "../../output", "testing_xgb")
    ...
    report_kfold_path = os.path.join(project_root, "../../output", "testing_xgb", "report_kfold.txt")
    kfold_results = parse_report(report_kfold_path)

    # The hold-out results are in `results`
    # We'll produce separate sets of plots for each hyper-tuning approach
    xgb_ht_options = sorted(list(set([ht for (_, _, ht) in results.keys()])))
    for ht in xgb_ht_options:
        holdout_filtered = {(nr, fe): results[(nr, fe, ht)]
                            for (nr, fe, tuning) in results.keys() if tuning == ht}
        # Filter K-Fold if available
        kfold_filtered = {}
        if kfold_results:
        ...
        # Create the heatmaps for hold-out vs K-Fold if we have them
        ...
        fig1.suptitle(f"XGB Test Metrics Heatmaps (HT: {ht})", fontsize=12)
        fig1.tight_layout(rect=[0, 0, 1, 0.95])
        heatmap_path = os.path.join(heatmaps_dir, f"xgb_{ht}_heatmap.png")
        fig1.savefig(heatmap_path, dpi=300)
        print(f"[XGB-TE] Saved combined heatmap to {os.path.relpath(heatmap_path, project_root)}")
        plt.close(fig1)

        # Create scatter/residual data from holdout_filtered
        ...
        fig2.suptitle(f"XGB Predicted vs. Actual (Hold-out) (HT: {ht})", fontsize=10)
        fig2.tight_layout(rect=[0, 0, 1, 0.95])
        pred_vs_actual_path = os.path.join(pred_vs_actual_dir, f"xgb_{ht}_predicted_vs_actual.png")
        fig2.savefig(pred_vs_actual_path, dpi=300)
        print(f"[XGB-TE] Saved predicted vs. actual plot to {os.path.relpath(pred_vs_actual_path, project_root)}")
        plt.close(fig2)
        ...
        fig3.suptitle(f"XGB Residual Plots (Hold-out) (HT: {ht})", fontsize=10)
        fig3.tight_layout(rect=[0, 0, 1, 0.95])
        residual_plot_path = os.path.join(residual_plots_dir, f"xgb_{ht}_residual_plot.png")
        fig3.savefig(residual_plot_path, dpi=300)
        print(f"[XGB-TE] Saved residual plot to {os.path.relpath(residual_plot_path, project_root)}")
        plt.close(fig3)
