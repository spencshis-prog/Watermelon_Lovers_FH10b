...
def build_xgb_model(hyper_tuning="default"):
    ...
    if hyper_tuning == "default":
        ...
        print(f"[XGB] Warning: 'bayesian' requested but not available. Using default XGBRegressor.")
        return XGBRegressor(random_state=42, eval_metric='rmse')


def kfold_train_and_evaluate_xgb(X, y, n_splits=5, epochs=20, batch_size=16, hyper_tuning="default"):
    """
    Performs K-fold cross-validation on (X, y) using an XGBoost model.
    (epochs, batch_size are placeholders for uniform interface.)
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        model = build_xgb_model(hyper_tuning)
        model.fit(
            X_train, y_train,
        ...
        if hasattr(model, 'best_estimator_'):
            # If it's a GridSearchCV / RandomizedSearchCV / BayesSearchCV, the best estimator is:
            final_est = model.best_estimator_
            y_pred = final_est.predict(X_val)
        else:
            # If it's a basic XGBRegressor
        ...
        print(f"[XGB] [Fold {fold_idx + 1}] MAE={mae:.2f}, RMSE={rmse:.2f}, R2={r2:.2f}")
...
def kfold_train_feature_set_xgb(feature_folder, models_output_dir,
    ...
    then trains a final XGB model on the remaining data.
    ...
    if os.path.exists(test_folder) and os.listdir(test_folder):
        ...
        print(
            f"[XGB] Using pre-split hold-out: {len(fnames_test)} test samples, {len(fnames_train_val)} training samples")
    else:
        ...
        print(f"[XGB] Hold-out test set created with {len(fn_test)} samples")

    # K-fold evaluation on the training+validation data
    robust_metrics = kfold_train_and_evaluate_xgb(X_train_val, y_train_val, n_splits=n_splits,
                                                  epochs=epochs, batch_size=batch_size,
                                                  hyper_tuning=hyper_tuning)
    mae_avg, mse_avg, rmse_avg, r2_avg = robust_metrics
    print(
        f"[XGB] [{nr_method} - {feat_method}] K-fold average -> MAE={mae_avg:.2f}, RMSE={rmse_avg:.2f}, R2={r2_avg:.2f}")
    ...
    print("[XGB] Final model parameters:", functions.relevant_params(final_model, "xgb", hyper_tuning))
    ...
    print(f"[XGB] Final model saved to {os.path.relpath(model_path, os.getcwd())}")
    return mae_avg, mse_avg, rmse_avg, r2_avg


def kfold_train_all_feature_models_xgb(feature_extraction_base_dir, models_output_dir, report_kfold_path,
    ...
    for nr in nr_folders:
        ...
        for feat in feat_folders:
            ...
            metrics = kfold_train_feature_set_xgb(feat_folder, models_output_dir,
    ...
    for (nr, feat, ht), (mae_avg, mse_avg, rmse_avg, r2_avg) in results.items():
        print(f"[XGB] {nr}-{feat} ({ht}) => MAE={mae_avg:.2f}, RMSE={rmse_avg:.2f}, R2={r2_avg:.2f}")
    print("[XGB] K-fold training completed for all feature sets.")

    # Append results to the K-Fold report
    ...
    print(f"[XGB] Report saved to {os.path.relpath(report_kfold_path, os.getcwd())}")
