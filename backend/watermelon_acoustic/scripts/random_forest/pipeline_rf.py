...
import functions
from scripts.random_forest.rf_optuna import kfold_train_feature_set_rf_optuna
# Import the RF training and testing functions from your RF modules.
from scripts.random_forest.rf_training import kfold_train_all_feature_models_rf
from scripts.random_forest.rf_testing import test_all_rf_models

# Configuration parameters: Hyperparameter tuning options for RF.
HT_OPTIONS = ["default", "grid", "random", "optuna"]
...
def proceed(TRAIN_NEW_MODELS=False):
    ...
    if TRAIN_NEW_MODELS:
        ...
        for ht in sorted(HT_OPTIONS):
            ...
            if ht == "optuna":
                kfold_train_feature_set_rf_optuna(
