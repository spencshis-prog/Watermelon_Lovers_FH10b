...
def test_model_on_holdout_rf(model_path, test_folder):
    """
    Loads a final RandomForest model and evaluates it on the hold-out test set.
    Expects test_folder to contain the held-out .npy feature files.
    Returns a dictionary of test metrics and predictions.
    """
    if not os.path.exists(test_folder):
        print(f"[RF-TE] Test folder {test_folder} does not exist. Skipping.")
        return None

    X, y, _ = load_feature_data(test_folder)
    if len(X) == 0:
        print("[RF-TE] No test data found.")
        return None
...
def test_all_rf_models(models_dir, feature_extraction_base_dir, report_path):
    """
    Iterates over each (NR, FE, hyper-tuning) combination in feature_extraction_base_dir,
    loads the corresponding final model (named rf_<NR>_<FE>_<ht>.pkl),
    ...
    for nr in nr_list:
        ...
        for fe in fe_list:
            ...
            for ht in ["default", "grid", "random", "optuna"]:
                model_name = f"rf_{nr}_{fe}_{ht}.pkl"
                model_path = os.path.join(models_dir, model_name)
                rel_model_path = os.path.relpath(model_path, project_root)
                if not os.path.exists(model_path):
                    print(f"[RF-TE] No model found for {nr}, {fe}, ht={ht} at {rel_model_path}. Skipping.")
                    continue
                print(
                    f"[RF-TE] Evaluating model {model_name} on test set from {os.path.relpath(test_folder, project_root)}")
                metrics = test_model_on_holdout_rf(model_path, test_folder)
    ...
    print(f"[RF-TE] Report saved to {rel_report_path}")

    plot_results_grid_rf(results, project_root)
...
def plot_results_grid_rf(results, project_root):
    """
    ...
    """
    base_save_dir = os.path.join(project_root, "../../output", "testing_rf")
    ...
    report_kfold_path = os.path.join(project_root, "../../output", "testing_rf", "report_kfold.txt")
    ...
    rf_ht_options = sorted(list(set([ht for (_, _, ht) in results.keys()])))
    for ht in rf_ht_options:
        print(f"[RF-TE] Plotting results for Hyper-tuning: {ht}")
        ...
        if kfold_filtered:
            ...
            heatmap_path = os.path.join(heatmaps_dir, f"rf_{ht}_heatmap.png")
            ...
            rel_heatmap_path = os.path.relpath(heatmap_path, project_root)
            fig1.savefig(heatmap_path, dpi=300)
            print(f"[TE] Saved hold-out heatmap to {rel_heatmap_path}")
            plt.close(fig1)
        ...
        pred_vs_actual_path = os.path.join(pred_vs_actual_dir, f"rf_{ht}_predicted_vs_actual.png")
        ...
        residual_plot_path = os.path.join(residual_plots_dir, f"rf_{ht}_residual_plot.png")
...
if __name__ == "__main__":
    ...
    models_dir = os.path.join(base_dir, "../../output", "models_rf")
    feature_extraction_base_dir = os.path.join(base_dir, "../../intermediate", "feature_extraction")
    report_path = os.path.join(base_dir, "../../output", "testing_rf", "report.txt")
    os.makedirs(os.path.dirname(report_path), exist_ok=True)
    test_all_rf_models(models_dir, feature_extraction_base_dir, report_path)
